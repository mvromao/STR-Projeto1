...
//#include <C:/STR/labwork1/labwork1.cpp>
...
//xSemaphoreHandle xSemP2 = xSemaphoreCreateCounting(1, 1);
...
void moveCylinderStartFront() {
	//xQueueSemaphoreTake(xSemP2, portMAX_DELAY);
...
void stopCylinderStart() {
	//xQueueSemaphoreTake(xSemP2, portMAX_DELAY);
	//printf("Before critical1 zone!\n");
	uInt8 p = readDigitalU8(2); // read port 2
	//printf("Before critical2 zone!\n");
	setBitValue(&p, 0, 0); // set bit 0 to low level
	//printf("Before critical3 zone!\n");
	setBitValue(&p, 1, 0); // set bit 1 to low level
	//printf("Before critical4 zone!\n");
	taskENTER_CRITICAL();
	writeDigitalU8(2, p); // update port 2
	taskEXIT_CRITICAL();
	//xSemaphoreGive(xSemP2);
