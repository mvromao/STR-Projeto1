...
// Tasks
// Verificar a integração do comando enviado pelo utilizador, agora ele recebe só 0 e 1 do Back e Front, porém precisa receber numeros de 0 a 7
...
void vTaskEnterBrick(void* pvParameters) {
	while (TRUE) {
		int readType = 1, userType = 1;
		xQueueReceive(mbx_entryBrick, &userType,portMAX_DELAY);
		printf("EnterBrick: User said brick type %d", userType);
